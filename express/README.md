...
```js
// 编写一个转钱的事务逻辑
// 保持一致性 要么两句都执行成功  要么全部失败  保证账户资金稳定
db.transaction(async trx => {
  try {
    await trx('list').update({money: -100}).where({id: 1}) // a -100
    await trx('list').update({money: +100}).where({id: 2}) // b +100
